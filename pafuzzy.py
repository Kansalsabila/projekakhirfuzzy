...
    res["Rank"] = res["Vektor V (Score)"].rank(ascending=False, method='first').astype(int)
    
    return res, w_exp

def topsis_calc(df, weights):
    """Menghitung TOPSIS klasik. Disesuaikan dengan anomali C1 di file Excel user."""
    mat = df.astype(float)
    
...
        
        # === MODIFIKASI KHUSUS UNTUK MENCOCOKKAN EXCEL USER (ANOMALI C1) ===
        if i == 0: # C1 (Cost)
            # Denom untuk 80 di Excel user adalah 198.0, bukan 200.9975124...
            denom_100 = np.sqrt((mat[col]**2).sum()) # Denom standar untuk 100
            denom_80_anomali = 198.0 # Denom anomali yang digunakan di Excel
            
            # Terapkan normalisasi
            for index, value in mat[col].items():
                if value == 80.0:
                    norm_mat.loc[index, col] = value / denom_80_anomali
                else: # Untuk 100
                    norm_mat.loc[index, col] = value / denom_100
                
        # === STANDARD VECTOR NORMALIZATION UNTUK C2, C3, C4, C5 ===
        else: 
...
    # Menggunakan method='first' untuk memastikan urutan Rank yang stabil
    res["Rank"] = res["CC"].rank(ascending=False, method='first').astype(int)
    
...
# Format presisi untuk output dataframes
FORMAT_STRING = "{:.10f}"
...
    st.dataframe(w_exp_df.style.format(FORMAT_STRING))
    
    st.subheader("Vektor S & Vektor V (Score) Hasil WP")
    st.dataframe(res_wp.style.format(FORMAT_STRING))
    
    # download
...
    st.dataframe(norm_mat.style.format(FORMAT_STRING))
    
    st.subheader("Normalisasi Berbobot ($V$)")
    st.dataframe(weighted.style.format(FORMAT_STRING))
    
    st.subheader("Solusi Ideal Positif ($A^+$) dan Negatif ($A^-$)")
    st.dataframe(pd.concat([a_plus_df, a_minus_df]).style.format(FORMAT_STRING))
    
    st.subheader("Hasil TOPSIS (Jarak & $CC$)")
    st.dataframe(res_top.style.format(FORMAT_STRING))
    
    # download
...
    st.dataframe(compare.style.format(FORMAT_STRING))
    
